...
export async function DELETE(
    ...
    try {
        ...
        // Delete from Cloudinary using our upload API
        try {
            const deleteResponse = await fetch(
                `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/upload?publicId=${encodeURIComponent(image.publicId)}&resourceType=auto`,
                {
                    method: 'DELETE',
                }
            )
            ...
            if (!deleteResponse.ok) {
                console.error('Cloudinary deletion failed:', await deleteResponse.text())
